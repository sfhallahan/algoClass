...
function getMaxProfit(array) {
	if (array.length < 2) {
		return "Array of stock prices contain more than two values";
	}
	let min = array[0];
...
/*Gotchas
  ...
  But that will take O(n^2), since we have two nested loops—for every time, we're going through every other time. Also, it's not correct: we won't ever report a negative profit! Can we do better?
